...
`helix-trade-backtest-pipe` is centered on trading systems work in Zig. The technical target is: Design a Zig verification harness for backtest systems, covering state machine modeling, transition tables, and failure-oriented tests.
...
The score is deterministic. The current threshold is 152. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 3.
